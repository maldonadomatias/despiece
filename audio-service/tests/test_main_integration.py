...
def client(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda audio, sr, bar_grid: [{"start_sec": 0.0, "end_sec": 1.0, "label": "Am"}],
    )
...
def test_analyze_returns_version_4_and_sub_label(client):
    resp = client.post(
        "/analyze",
...
def test_analyze_returns_version_3_when_drum_hits_enabled(monkeypatch):
    ...
    # Chords disabled for this test so version stays at 3 (drum hits present, no chords)
    monkeypatch.setattr(
        main_mod, "detect_chords",
...
def test_analyze_returns_version_2_when_drum_hits_disabled(monkeypatch):
    ...
    # Both drum hits disabled and chords returning [] so version stays at 2
    monkeypatch.setattr(
        main_mod, "detect_chords",
        lambda audio, sr, bar_grid: [],
    )
